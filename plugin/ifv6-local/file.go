...
var log = clog.NewWithPlugin("ifv6_local")
...
func (f File) Name() string { return "ifv6_local" }
...
func Parse(f io.Reader, origin, fileName string, serial int64) (*Zone, error) {
	...
	if !seenSOA {
		return nil, fmt.Errorf("ifv6_local %q has no SOA record for origin %s", fileName, origin)
	}
